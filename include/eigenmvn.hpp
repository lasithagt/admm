...
// Edited 2016 by Kamil Saigol for fixed-size matrices
...
{
    Matrix<Scalar,Dim,Dim> _covar;
    Matrix<Scalar,Dim,Dim> _transform;
    Matrix< Scalar, Dim, 1> _mean;
    internal::scalar_normal_dist_op<Scalar> randN; // Gaussian functor
    bool _use_cholesky;
    SelfAdjointEigenSolver<Matrix<Scalar,Dim,Dim> > _eigenSolver; // drawback: this creates a useless eigenSolver when using Cholesky decomposition, but it yields access to eigenvalues and vectors

public:
    EigenMultivariateNormal(const Matrix<Scalar,Dim,1>& mean, const Matrix<Scalar,Dim,Dim>& covar,
    ...
    void setMean(const Matrix<Scalar,Dim,1>& mean) { _mean = mean; }

    void setCovar(const Matrix<Scalar,Dim,Dim>& covar)
    {
        _covar = covar;
        ...
        if (_use_cholesky)
        {
            Eigen::LLT<Eigen::Matrix<Scalar,Dim,Dim> > cholSolver(_covar);
        ...
        {
            _eigenSolver = SelfAdjointEigenSolver<Matrix<Scalar,Dim,Dim> >(_covar);
     ...
     * as columns in a Dim by nn matrix
     */
    Matrix<Scalar,Dim,-1> samples(int nn) const
    {
        return (_transform * Matrix<Scalar,Dim,-1>::NullaryExpr(_covar.rows(),nn,randN)).colwise() + _mean;
    }
