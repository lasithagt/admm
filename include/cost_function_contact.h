...
struct ContactTerms
{
    ...
    Eigen::Vector2d computeContactTerms(const stateVec_t& x, double R_c)
    {
        ...
        contactTerms(0) = mass * sqrt(vel(0) * vel(0) + vel(1) * vel(1)) / R_c;
        contactTerms(1) = x(16);
    ...
    Eigen::VectorXd contact_x(const stateVec_t& x, double R_c) 
    {
        ...
        CX.head(7)      = 2 * mass * (1/R_c) * vel.transpose() * getContactJacobianDot(q, qd).block(0,0,3,NDOF);
        CX.segment(7,7) = 2 * mass * (1/R_c) * vel.transpose() * getContactJacobian(q).block(0,0,3,NDOF);
        return CX;
    }

    /* compute the hessian */
    Eigen::MatrixXd contact_xx(const stateVec_t& x, double R_c) 
    {
        // TODO: optimize this part
        // Assumption: 
        memcpy(q, x.head(7).data(), 7 * sizeof(double));
