...
public:
    ...
    KUKASoftContactFDSystem(const KUKASoftContactFDSystem& arg) : BASE(arg)
    {
        ...
        contactModel_ = std::shared_ptr<SoftContactModel_t>();
    }
        ...
        state_vector_t& derivative) override
    {
        ...
        RigidBodyPose_t pose_ee = kinematics_->getEEPoseInWorld(ee_id, this->basePose_, robotState.joints().getPositions());

        
        // Kinematics, velocity
        Eigen::Matrix<SCALAR, 3, 1> velocity_ee = kinematics_->getEEVelocityInBase(ee_id, robotState.toRBDState()).vector();
        Eigen::Matrix<SCALAR, 6, 7> jac = kinematics_->getJacobianBaseEEbyId(ee_id, robotState.toRBDState());
        Eigen::Matrix<SCALAR, 3, 1> accel_ee = jac.template bottomRows<3>() * jAcc.getAcceleration();

        Eigen::Matrix<SCALAR, 3, KUKA::Kinematics::NJOINTS> Jc_Rotational, Jc_Translational, dJdt;
        ...
        // poseM = pose_ee.getRotationMatrix().matrix();

        // compute the cartesian space mass matrix
        Eigen::Matrix<SCALAR, 3, 3> massMatrix;
        // massMatrix << 1,0,0,0,1,0,0,0,1;
        vector3_t df_;
        vector3_t orientation;
        vector3_t force_current;

        // compute contact dynamics 
        contactModel_->df(massMatrix, pose_ee.position().vector(), orientation, velocity_ee, accel_ee, force_current, df_);
