...
const typename iit::Kuka::dyn::tpl::InverseDynamics<TRAIT>::ExtForces
iit::Kuka::dyn::tpl::InverseDynamics<TRAIT>::zeroExtForces(Force::Zero());

template <typename TRAIT>
iit::Kuka::dyn::tpl::InverseDynamics<TRAIT>::InverseDynamics(IProperties& inertia, MTransforms& transforms) :
    ...
    link7_I(inertiaProps->getTensor_link7() )
    {
#ifndef EIGEN_NO_DEBUG
    std::cout << "Robot Kuka, InverseDynamics<TRAIT>::InverseDynamics()" << std::endl;
...
template <typename TRAIT>
void iit::Kuka::dyn::tpl::InverseDynamics<TRAIT>::id(
    JointState& jForces,
...
template <typename TRAIT>
void iit::Kuka::dyn::tpl::InverseDynamics<TRAIT>::G_terms(JointState& jForces)
{
    // Link 'link1'
    link1_a = (xm->fr_link1_X_fr_base0).col(iit::rbd::LZ) * Scalar(iit::rbd::g);
    ...
    // Link 'link7'
    link7_a = (xm->fr_link7_X_fr_link6) * link6_a;
    link7_f = link7_I * link7_a;

    secondPass(jForces);
}

template <typename TRAIT>
void iit::Kuka::dyn::tpl::InverseDynamics<TRAIT>::C_terms(JointState& jForces, const JointState& qd)
{
    // Link 'link1'
    ...
    // Link 'link7'
    link7_v = ((xm->fr_link7_X_fr_link6) * link6_v);
    link7_v(iit::rbd::AZ) += qd(JG);
    
    iit::rbd::motionCrossProductMx<Scalar>(link7_v, vcross);
    
    link7_a = (xm->fr_link7_X_fr_link6) * link6_a + vcross.col(iit::rbd::AZ) * qd(JG);
    
    link7_f = link7_I * link7_a + iit::rbd::vxIv(link7_v, link7_I);
...
template <typename TRAIT>
void iit::Kuka::dyn::tpl::InverseDynamics<TRAIT>::firstPass(const JointState& qd, const JointState& qdd, const ExtForces& fext)
{
    ...
    link7_f = link7_I * link7_a + iit::rbd::vxIv(link7_v, link7_I) - fext[LINK7];
    
}

template <typename TRAIT>
void iit::Kuka::dyn::tpl::InverseDynamics<TRAIT>::secondPass(JointState& jForces)
{
    // Link 'link7'
    jForces(JG) = link7_f(iit::rbd::AZ);
    link6_f += xm->fr_link7_X_fr_link6.transpose() * link7_f;
    // Link 'link6'
