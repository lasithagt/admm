...
template<typename RobotModel, int S, int C>
void ADMMTrajOptimizerMPC<RobotModel, S, C>::run(const std::shared_ptr<RobotModel>& kukaRobot, std::shared_ptr<ADMMTrajOptimizerMPC<RobotModel, S, C>::PlantPublisher>& plantPublisher,  const stateVec_t& init_state,  const ContactModel::SoftContactModel<double>& contactModel,
  const ADMM_MPCconfig& ADMM_MPC_config, const TrajectoryDesired<S, NumberofKnotPt>& desiredTrajectory,  optimizer::IterativeLinearQuadraticRegulatorADMM::traj& result) 
{
...
template<typename RobotModel, int S, int C>
optimizer::IterativeLinearQuadraticRegulatorADMM::traj ADMMTrajOptimizerMPC<RobotModel, S, C>::getOptimizerResult() 
{
