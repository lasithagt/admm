...
void ILQRSolver_TRK::solveTrajectory()
{
    ...
    for(iter=0;iter<Op.max_iter;iter++)
    {
        ...
        // std::cout << "\nlasitha\n" <<std::endl;
        // std::cout << uListFull[N].transpose() << std::endl;
...
void ILQRSolver_TRK::doForwardPass()
{
 ...
 // sqrt(-1)=NaN. After this line, u_nan has nan for [0] and garbage for rest
    isUNan = 0;
    ...
    if (!initFwdPassDone)
    {
        ...
        for (unsigned int i = 0;i < N; i++) 
        {
            ...
            dynamicModel->kuka_arm_dyn_cst_min_output(i, dt, updatedxList[i], updateduList[i], xList_bar[i], uList_bar[i], isUNan, updatedxList[i+1], costFunction);
            costList[i] = costFunction->getc();
        }
        // getting final cost, state, input=NaN
        isUNan = 1;
        dynamicModel->kuka_arm_dyn_cst_min_output(N, dt, updatedxList[N], u_NAN_loc, xList_bar[N], u_NAN_loc, isUNan, x_unused, costFunction);
    ...
    // std::cout << updatedxList[N].transpose() << std::endl;   
}
